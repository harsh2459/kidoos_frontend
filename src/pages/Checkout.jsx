...
export default function Checkout() {
  ...
  async function placeWithoutPayment() {
    ...
    try {
      ...
      if (data?.ok) {
        setPlaced({ orderId: data.orderId || data._id || "created" });
        clear();
      } else {
    ...
    <div className="mx-auto max-w-screen-xl px-4 py-8 grid lg:grid-cols-3 gap-6">
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-6 shadow-sm">
        ...
        <div className="space-y-3">
          {items.map((i) => (
            <div key={(i._id || i.id) + String(i.qty)} className="flex gap-3">
              ...
              <button className="text-red-600 text-sm" onClick={() => remove(i._id || i.id)}>
                Remove
              </button>
            </div>
          ))}
        ...
        <div className="mt-4 space-y-2">
          {hasOnlinePay ? (
            <button disabled className="w-full btn-muted">
