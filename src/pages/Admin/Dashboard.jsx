...
export default function Dashboard() {
  ...
  if (loading && !data) return <div className="p-10 flex justify-center"><Loader2 className="animate-spin text-[#222831]" /></div>;
        ...
        <div className="flex gap-2 bg-white p-1.5 rounded-xl border-2 border-[#E3E8E5] shadow-sm">
          {['today', 'week', 'month', 'all'].map(p => (
            <button
              ...
              className={`px-5 py-2.5 rounded-lg text-sm font-bold capitalize transition-all duration-200 ${period === p ? 'bg-gradient-to-r from-[#222831] to-[#393E46] text-white shadow-md' : 'text-[#5C756D] hover:bg-gray-50'}`}
            >
              {p === 'all' ? 'All Time' : p}
            </button>
          ))}
            ...
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={analytics.trend}>
                ...
                <YAxis
                  ...
                  dx={-10}
                />
                <Tooltip cursor={{ fill: '#88BDF2', opacity: 0.1 }} contentStyle={{ backgroundColor: '#fff', border: '1px solid #E3E8E5', borderRadius: '12px', color: '#222831', fontSize: '12px', fontWeight: 'bold', padding: '8px 12px' }} />
