import { useCustomer } from "../../contexts/CustomerAuth";
import { useCart } from "../../contexts/CartStore"; // Import Cart Store
import { CustomerAPI } from "../../api/customer"; // Import API
import { t } from "../../lib/toast";
import { useNavigate, useLocation } from "react-router-dom";

export default function GoogleLoginButton({ onSuccess }) {
  const { googleLogin, token } = useCustomer(); // Ensure you get token from context if available immediately
  const { replaceAll } = useCart(); // Get cart actions
  const navigate = useNavigate();
  const location = useLocation();
  
  const nextPath = location.state?.next || "/";
  const pendingItem = location.state?.pendingItem; // Get pending item

  const handleClick = async () => {
    try {
      // 1. Perform Login
      const data = await googleLogin(); // Assumes this returns { token, customer }
      const newToken = data?.token;

      t.success("Logged in with Google!");

      // 2. Check for Pending Item (Buy Now flow)
      if (pendingItem && newToken) {
        try {
          // Add the pending item to the backend cart immediately
          const res = await CustomerAPI.addToCart(newToken, {
            bookId: pendingItem.bookId,
            qty: pendingItem.qty
          });
          
          // Update local cart state
          if (res?.data?.cart?.items) {
            replaceAll(res.data.cart.items);
          }
        } catch (cartErr) {
          console.error("Failed to add pending item:", cartErr);
          t.err("Could not add item to cart");
        }
      }

      // 3. Navigate
      if (onSuccess) {
        onSuccess();
      } else {
        navigate(nextPath, { replace: true });
      }
    } catch (error) {
      console.error("Google login failed:", error);
      t.err("Google login failed");
    }
  };

  return (
    <button
      onClick={handleClick}
      type="button"
      className="w-full flex items-center justify-center gap-3 px-4 py-3.5 border border-gray-300 rounded-xl bg-white text-gray-700 font-medium hover:bg-gray-50 transition-all shadow-sm hover:shadow-md active:scale-[0.99] group"
    >
      <div className="flex items-center justify-center w-6 h-6 shrink-0">
        <svg className="w-5 h-5" viewBox="0 0 24 24">
            <path fill="#4285F4" d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z" />
            <path fill="#34A853" d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z" />
            <path fill="#FBBC05" d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z" />
            <path fill="#EA4335" d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z" />
        </svg>
      </div>
      <span className="text-base leading-none pt-0.5">Continue with Google</span>
    </button>
  );
}
